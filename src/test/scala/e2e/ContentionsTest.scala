...
class ContentionsTest extends FlatSpec with Matchers {
  ...
  val sleepTime = 1000
  val Balance = 100
  val Executionsnumber = 16       //x*10+6
  ...
  "Contention test patten 1 a->b, c->d, c->e, One order" should "be handled okay" in {
    ...
    var t1 = new Transaction(Operation.set, "a", None, immutable.Seq[String](), immutable.Seq[String]("a"),Balance, Application.A)
    var transactionMessage1 = RequestMessage(t1, 0, "cl1", "ord1")

    var t2 = new Transaction(Operation.transfer, "a", Some("b"), immutable.Seq[String]("a"), immutable.Seq[String]("b"),1, Application.B)
    var transactionMessage2 = RequestMessage(t2, 0, "cl1", "ord1")

    val t3 = new Transaction(Operation.set, "c", None, immutable.Seq[String](), immutable.Seq[String]("c"),Balance, Application.C)
    val transactionMessage3 = RequestMessage(t3, 0, "cl1", "ord1")

    var t4 = new Transaction(Operation.transfer, "c", Some("d"), immutable.Seq[String]("c"), immutable.Seq[String]("d"),1, Application.D)
    var transactionMessage4 = RequestMessage(t4, 0, "cl1", "ord1")

    var t5 = new Transaction(Operation.transfer, "c", Some("e"), immutable.Seq[String]("c"), immutable.Seq[String]("e"),1, Application.E)
    var transactionMessage5 = RequestMessage(t5, 0, "cl1", "ord1")
    ...
    order.currentBlock.getTransactions.length shouldBe 0              //The test number is still not clear to me, Still the number of messages and the finished blocks number has a mismatch
    val aRes = 100-(Executionsnumber)/Number_of_transfers
    val bRes = (Executionsnumber)/Number_of_transfers
    val cRes = 100-(Executionsnumber)/Number_of_transfers
    val dRes = (Executionsnumber)/Number_of_transfers
    val eRes = (Executionsnumber)/Number_of_transfers
  ...
  "Contention test patten 2 a -> b -> c -> d -> e, One order" should "be handled okay" in {
    ...
    var t5 = new Transaction(Operation.transfer, "d", Some("e"), immutable.Seq[String]("d"), immutable.Seq[String]("e"),1, Application.E)
    var transactionMessage5 = RequestMessage(t5, 0, "cl1", "ord1")
    ...
    val cRes = 100-(Executionsnumber)/Number_of_transfers
    val dRes = 1
    val eRes = (Executionsnumber)/Number_of_transfers - 1
    for (ex <- Array(ex1, ex2, ex3, ex4, ex5)) {
      ex.state.getBalance("a") shouldBe Some(aRes)
      ex.state.getBalance("b") shouldBe Some(bRes)
      ex.state.getBalance("c") shouldBe Some(cRes)
