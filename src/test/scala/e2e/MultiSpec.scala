...
class MultiSpec extends FlatSpec with Matchers {
  ...
  "Two transactions, 1 order, 2 executor for different applications" should "function properly" in {
    ...
    for (w <- 0 to 9) {
      val t1 = new Transaction(Operation.set, "a", None, immutable.Seq[String](), immutable.Seq[String]("a"),w, Application.A)
    ...
    for (w <- 0 to 9){
      val t2 = new Transaction(Operation.set, "b", None, immutable.Seq[String](), immutable.Seq[String]("b"),w, Application.B)
