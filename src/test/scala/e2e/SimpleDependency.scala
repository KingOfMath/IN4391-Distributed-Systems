...
class SimpleDependency extends FlatSpec with Matchers {
  ...
  "One orderer and 3 executor, trying the dependency issue" should "be handled okay" in {
    ...
    var t1 = new Transaction(Operation.set, "a", None, immutable.Seq[String](), immutable.Seq[String]("a"),aBalance, Application.A)
    ...
    for (w <- 2 to 9) {
      val t3 = new Transaction(Operation.set, "c", None, immutable.Seq[String](), immutable.Seq[String]("c"),w, Application.C)
