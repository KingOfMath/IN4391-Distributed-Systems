...
class OrderSpec extends FlatSpec with Matchers {
  ...
  "Orderer" should "be able to add transactions to a block" in {
    ...
    val t1 = new Transaction(Operation.set, "test1", None, immutable.Seq("test1"), immutable.Seq("test1"), 1, Application.A)
    val t2 = new Transaction(Operation.transfer, "test1", Some("test2"), immutable.Seq("test1", "test2"), immutable.Seq("test2"), 1, Application.A)
  ...
  "Orderer" should "be able to generate a correct DependencyGraph" in {
    ...
    val t1 = new Transaction(Operation.set, "test1", None, immutable.Seq("test1"), immutable.Seq("test1"), 1, Application.A)
    val t2 = new Transaction(Operation.transfer, "test1", Some("test2"), immutable.Seq("test1", "test2"), immutable.Seq("test2"), 1, Application.A)
    val t3 = new Transaction(Operation.set, "test3", None, immutable.Seq("test3"), immutable.Seq("test3"), 3, Application.A)
    val t4 = new Transaction(Operation.transfer, "test2", Some("test3"), immutable.Seq("test2", "test3"), immutable.Seq("test2", "test3"), 1, Application.A)
    // t2 should depend on t1 and t4 should depend on t2 and t3, no other dependencies should exist
    order.addTransactionToBlock(t1)
    order.addTransactionToBlock(t2)
